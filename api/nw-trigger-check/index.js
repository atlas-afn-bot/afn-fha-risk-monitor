 ...
 *   { triggered:false, alreadyTriggered:true, month } — marker exists
 *   { triggered:true, month, aaWorkItemId?, message } — bot enqueued
...
async function handleRequest(context, req, deps = {}) {
  ...
  // 2) Idempotency
  if (await markerExists(containerClient, month)) {
  ...
  context.log.info(
    `nw-trigger-check: enqueued month=${month} workItemId=${enqueueResult && enqueueResult.aaWorkItemId ? enqueueResult.aaWorkItemId : 'n/a'} corr=${correlationId}`,
  );
  ...
  return jsonResponse(context, 200, {
    ...
    ...(enqueueResult && enqueueResult.aaWorkItemId
      ...
      : {}),
    message: `Neighborhood Watch bot enqueued for ${monthNameFor(month)}.`,
  });
}
...
module.exports._internals = { validateMonth, monthNameFor, scanMonth, markerExists };
