// PR-3 (Projected Drop-Off) PDF capture script. Mirrors capture-pr23-pdf.mjs
// but writes the output PDF to verification/PR-3-projected-dropoff.pdf so
// reviewers can visually inspect the Projected Drop-Off section layout.
...
(async () => {
  ...
  const pdfPath = path.join(OUT_DIR, 'PR-3-projected-dropoff.pdf');
  await download.saveAs(pdfPath);
  console.log('[capture] saved PDF', pdfPath);

  const pngPath = path.join(OUT_DIR, 'PR-3-projected-dropoff-page1.png');
